...
> Global runtime safety note: this copy is a local fork. Follow `SKILL.md`, use
> environment variables for secrets, and do not use `--api-key` or
> `toggle_builtin_tools`. Those upstream interfaces are intentionally not
> exposed by the global CLI.
