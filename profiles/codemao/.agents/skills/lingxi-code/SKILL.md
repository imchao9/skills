---
name: lingxi-code
description: Implement, modify, fix, or review Java/Kotlin backend code with mixed enforcement (hard constraints + soft suggestions). Use for backend service/API/mapper/repository/config implementation tasks, review-only checks, codegen-min-spec compatibility triggers, and tasks that provide a lingxi-plan RequirementContext or backend technical plan.
---

# 灵犀·代码实现

## Goal
Apply the minimum non-negotiable specification for Java/Kotlin backend implementation and review.
...
Preferred prompts:
- `使用 $lingxi-code 实现：新增用户分页查询接口`
- `使用 $lingxi-code 审查当前后端改动`
- `使用 $lingxi-code 实现：只改 service 和 mapper，不改接口协议`
- `使用 $lingxi-code 基于以下 RequirementContext 和后端技术方案实现：...`
...
- Explicit trigger: User asks to use `lingxi-code`, `$lingxi-code`, `codegen-min-spec`, or `$codegen-min-spec`.
...
- Requirement trigger: User provides structured `RequirementContext` from `lingxi-plan`.

Apply this skill to:
- Java/Kotlin backend implementation tasks.
- Existing Java/Kotlin backend changes and new code additions.
- Implementation planning and plan-to-code mapping needed to execute backend changes.
- Review-only checks for Java/Kotlin backend changes.

Do not apply this skill as frontend/UI-specific guidance.
When the user only needs requirement collection or a technical plan, use `lingxi-plan`; planning inside this skill exists only to support code implementation.
...
- Recommend `Plan-Full` for high-risk, cross-module, DB-write, external-call, schema/data mutation, ES, or broad implementation work.
...
- For `RequirementContext` / `SolutionContext` handling, load [references/lingxi-plan-intake.md](references/lingxi-plan-intake.md).
...
- Load [references/lingxi-plan-intake.md](references/lingxi-plan-intake.md) when the user provides `RequirementContext`, asks to implement from an upstream technical plan, or needs plan-to-code mapping.
