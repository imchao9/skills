...
- `需求清单路径`、`需求收集单路径`、`后端技术方案路径` 均存在且可读
- `blocking_source_gaps=[]`
Expected:
- 读取后端技术方案并进入 `Implement Mode`。
- 使用 `RequirementContext` 做范围和追溯元数据，不重新读取原始需求 URL。

### Case 63: 全栈完成态交接（仅实现后端）
Input:
- `implementation_surface=fullstack`
- 前后端方案均已生成，所有完成态门禁通过
Expected:
- 只消费 `后端技术方案路径` 并实现后端范围。
- 输出明确说明前端实现不在本技能交付范围内，不得宣称全栈实现完成。
...
Input:
- `implementation_surface=backend`
- `backend_plan_status=preview_only` 或 `pending`
Expected:
- 停止实现，要求先回到“灵犀·需求方案”完成后端技术方案。

### Case 66: 后端技术方案路径缺失或不可读（阻塞）
Input:
- 完成态枚举均通过
- `后端技术方案路径` 缺失、文件不存在或不可读
Expected:
- 停止实现并指出具体路径问题。
- 不使用旧字段 `技术方案路径` 兜底。
...
### Case 67: 来源缺口未授权（阻塞）
Input:
- `blocking_source_gaps` 非空
- `source_gap_approval=pending`
Expected:
- 停止实现并要求确认来源缺口。

### Case 68: 来源缺口已授权（带风险继续）
Input:
- `source_completeness=partial`
- `blocking_source_gaps` 非空
- `source_gap_approval=approved`
- 其他完成态门禁通过
Expected:
- 仅在已授权范围内继续实现。
- 在实施摘要中保留来源不完整风险。

### Case 69: 无 RequirementContext 的直接实现（通过）
Input:
- 用户直接要求实现一个范围明确的 Java/Kotlin 后端小改动。
Expected:
- 按直接任务和仓库证据进入实现流程。
- 不伪造 `RequirementContext` 或上游完成状态。
