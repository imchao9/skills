...
```text
使用 $cg-req 读取这个钉钉需求：https://docs.dingtalk.com/i/nodes/<nodeId>，项目在 /path/to/project
```
...
```text
使用 $cg-req 读取本地方案：docs/features/foo_需求清单.md，项目在 /path/to/project，生成技术方案
```
...
```text
使用 $cg-req 对比钉钉 https://docs.dingtalk.com/i/nodes/<nodeId> 和本地 docs/features/foo_技术方案.md，冲突先问我
```
...
- `technical_plan_status=pending`，不推荐进入实现或 cg-gen。
...
- 若影响实现决策，写入 `blocking_source_gaps`，`technical_plan_status=pending`，不推荐进入实现或 cg-gen。
